...
@implementation CLDrawTool
{
    ...
    UIImageView *_colorPreviewBackground;
...
- (void)setMenu
{
    ...
    _strokePreview = [[UIView alloc] initWithFrame:CGRectMake(0, 0, W/2 - 5, W/2 - 5)];
...
//    _strokePreview.layer.borderColor = [[[CLImageEditorTheme theme] toolbarTextColor] CGColor];
    ...
    _strokePreview.center = CGPointMake(_menuView.width-W/2, _menuView.height*3/4);
    ...
    UIImage* clrbgImg = [UIImage imageNamed:@"camera_clrbg"];
    _colorPreviewBackground = [[UIImageView alloc] initWithImage:clrbgImg];
    _colorPreviewBackground.frame = CGRectMake(0, 0, W/2 - 5, W/2 - 5);
    _colorPreviewBackground.center= CGPointMake(_menuView.width-W/2,_menuView.height/4);
    [_menuView addSubview:_colorPreviewBackground];
    
    _colorPreview = [[UIView alloc] initWithFrame:CGRectMake(0, 0, W/6, W/6)];
    _colorPreview.layer.cornerRadius = _colorPreview.height/2;
    _colorPreview.center = CGPointMake(_menuView.width-W/2, _menuView.height/4);
    [_menuView insertSubview:_colorPreview belowSubview:_colorPreviewBackground];
...
- (void)colorSliderDidChange:(UISlider*)sender
{
    if(_eraserIcon.hidden){
        _colorPreview.backgroundColor = [self colorForValue:_colorSlider.value];
...
-(void)drawLine:(CGPoint)from to:(CGPoint)to
{
    ...
    UIColor *strokeColor = _colorPreview.backgroundColor;//_strokePreview.backgroundColor;
